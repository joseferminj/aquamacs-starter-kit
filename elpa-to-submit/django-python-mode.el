...
(add-hook 'django-python-mode-hook
          (lambda ()
            (font-lock-add-keywords nil
                                    '(("\\<\\(request\\.\\(POST\\|GET\\|method\\|is_ajax\\|user\\(\\.[a-z_]+\\)\\{0,\\}\\)\\|request\\)" . font-lock-constant-face)
                                      ...
                                      ("\\<\\(get_list_or_404\\|get_object_or_404\\|load_on_render\\|loader\\|reverse\\|HttpResponseRedirect\\|RequestContext\\|render_to_response\\)" . font-lock-function-name-face)
                                      ("\\<[a-zA-Z_]+\\.\\(get_\\(object\\|list\\|iterator\\|count\\|values\\|values_iterator\\|in_bulk\\|or_create\\|absolute_url\\)\\)" 1 font-lock-function-name-face))
                    ))
          )
